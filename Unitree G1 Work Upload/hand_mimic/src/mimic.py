...
def _screen_size() -> tuple[int, int]:
    """Best-effort primary-monitor resolution (w, h). Tries xrandr, then
    tkinter, then falls back to 1920x1080. Used to force-fill the window since
    GNOME/mutter doesn't reliably honour OpenCV's WND_PROP_FULLSCREEN."""
    import subprocess
    try:
...
def _apply_fullscreen(win: str, on: bool) -> None:
    """Make ``win`` fill the screen (or restore it). Sets WND_PROP_FULLSCREEN
    *and* explicitly resizes/moves the window, because the fullscreen property
    alone is silently ignored by GNOME/mutter (Qt backend) — the property is
    honoured by some WMs, the explicit resize+move covers the rest."""
    if on:
        sw, sh = _screen_size()
...
def main() -> int:
    ...
    win_shown  = False   # defer fullscreen until after the first imshow (WM reliability)
    screen_wh  = _screen_size()   # fill target when fullscreen (whole screen)
    try:
        win_wh = tuple(int(v) for v in args.window_size.lower().split("x"))
    except Exception:
        win_wh = (1280, 960)      # fill target when windowed
    ...
    if not args.no_window:
        ...
        try:
            ...
            cv2.resizeWindow(win, 1280, 960)
        # NOTE: fullscreen is applied after the first imshow (below). Setting
        # WND_PROP_FULLSCREEN before any frame is shown is silently ignored by
        # many window managers, leaving the window at its resized size.
    ...
    try:
        while True:
            ...
            if args.no_window:
                ...
                # Stretch the frame to the full window (whole screen in
                # fullscreen) so the image fills it edge-to-edge with no
                # gray/black margins. Independent of the WM honouring the
                # fullscreen property.
                disp_wh = screen_wh if fullscreen else win_wh
                disp = (cv2.resize(frame, disp_wh)
                        if (frame.shape[1], frame.shape[0]) != disp_wh else frame)
                cv2.imshow(win, disp)
                if not win_shown:
                    # Apply fullscreen now that the window has real content — this
                    # is what actually makes it span the whole screen.
                    win_shown = True
                    if fullscreen:
                ...
                if key == ord("f"):
                    ...
                    _apply_fullscreen(win, fullscreen)
