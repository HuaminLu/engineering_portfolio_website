...
class GeoffWindow(QtCore.QObject):  # type: ignore[misc]  # pylint: disable=too-few-public-methods
    def __init__(
        ...
        *,
        ...
        # GL viewer for point-cloud — skip gracefully if OpenGL unavailable (X11 forwarding)
        try:
            ...
            log.warning("3-D point-cloud view disabled (no OpenGL): %s", _gl_exc)
            self.gl_view = QtWidgets.QLabel("3-D view unavailable (no OpenGL)")
            self.gl_view.setMinimumWidth(640)
            self._scatter = None
            self._pose_items = []
        ...
        # the streams.
        # Boot sequence disabled — robot must already be in balance mode
        self._bot = None
        ...
        try:
            from unitree_sdk2py.core.channel import ChannelFactoryInitialize as _CFI, ChannelPublisher  # type: ignore
            _CFI(0, iface)
...
# need the separate `_keyboard_thread`.
    ...
    # Learned-arm control (averaged-delta model) --------------------------
    # Keys that drive the arm via inference (I/J/K/L/U/O).  Kept distinct from
    # the tele-op drive keys (W/A/S/D/Q/E) so the two never collide.
    _ARM_KEYS = frozenset({"i", "j", "k", "l", "u", "o"})
    # Fraction of the averaged delta applied per keypress (1.0 = full move).
    _ARM_GAIN = 1.0
    # Ramp step (rad/tick) used while gliding to a learned target — ~40% of the
    # default 0.02 for a slow, safe test.  Raise once behaviour is trusted.
    _ARM_LEARN_STEP = 0.008
    ...
    def eventFilter(self, _obj, ev):  # type: ignore[override]
        ...
        if ev.type() == QtCore.QEvent.KeyPress:
            ...
            if name is not None:
                    ...
                    # ----------------------------------------------------
                    # Learned arm control (I/J/K/L/U/O) and hand control are
                    # mutually exclusive so a key like 'o' never triggers both.
                    # ----------------------------------------------------
                    if name in self._ARM_KEYS:
    ...
    def _qt_key_name(key: int, text: str | None) -> str | None:
        ...
        if text:
            ch = text.lower()
            # w/a/s/d/q/e -> tele-op drive; i/j/k/l/u/o -> learned arm control;
            # f/b legacy arm; p -> hand close; g/h legacy hand.
    ...
    def _maybe_arm_inference(self, key_name: str) -> None:  # noqa: D401
        ...
        # Map our canonical key names → direction string.  Arm control now
        # lives on I/J/K/L/U/O (W/A/S/D/Q/E stay reserved for tele-op drive):
        #   I=up  K=down  J=left  L=right  U=forward  O=back
        dir_map = {
            "i": "up",
            "k": "down",
            "j": "left",
        ...
        # ----------------------------------------------------------------
        #  Pre-conditions – we need the arm_sdk publisher.
        # ----------------------------------------------------------------
        ...
        try:
            # Averaged-delta model: target = current + gain * mean_delta[dir].
            # Lightweight (joblib + numpy) so import cost is negligible.
            from data.inference_arm import predict_delta_target, load_deltas  # type: ignore

            # Cache the loaded bundle per arm so repeated presses are instant.
            if not hasattr(self, "_arm_delta_cache"):
                self._arm_delta_cache = {}

            if self._active_arm not in self._arm_delta_cache:
                from pathlib import Path

                bundle_path = Path(
                    f"data/artifacts/{self._active_arm}-arm/arm_deltas.joblib"
                )
                try:
                    self._arm_delta_cache[self._active_arm] = load_deltas(
                        bundle_path, arm=self._active_arm
                    )
                except Exception:
                    # Fall back to the helper's default search.
                    self._arm_delta_cache[self._active_arm] = load_deltas(
                        arm=self._active_arm
                    )

            bundle = self._arm_delta_cache[self._active_arm]

            # Current joint angles as the *start* pose: prefer measured
            # LowState, else the last commanded values.  The delta is applied
            # relative to this, so repeated presses keep stepping the arm.
            current = [
            ...
            # Slow the ramp for learned moves so the arm glides to the target
            # (sentdex's "always use a gradient — smooth and slow" warning).
            self._STEP = self._ARM_LEARN_STEP

            # One-step pose; _on_arm_tick ramps _cmd_q toward it.  Waist held.
            target_pose = [(self._waist_idx, self._cmd_q.get(self._waist_idx, 0.0))]
            target_pose += list(zip(sorted(self._arm_joint_idx), target))
